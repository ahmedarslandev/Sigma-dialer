    ...
    try {
      ...
      if (!userId || !trimmedCallerId) {
        return res.status(400).json({
          message: "userId or callerId not found",
          status: 400,
        });
      }
      ...
      let callerIdNumber;
      if (!state) {
        const callerIds = await callerIdModel.find({ owner: user._id });
        callerIdNumber = callerIds.length
          ? callerIds[Math.floor(Math.random() * callerIds.length)].Number
          : null;
      } else {
        const callerIds = await callerIdModel.find({
        ...
        if (callerIds.length) {
          ...
          callerIdNumber = user.callerIds.length
            ? user.callerIds[Math.floor(Math.random() * user.callerIds.length)]
                .Number
            : null;
        }
      }
